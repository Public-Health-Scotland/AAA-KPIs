...
#reorder rows
kpi_1.4a_assess <- template %>% left_join(kpi_1.4a_assess,  by = c("fy_due", "hbres" = "hbres_final"))
...
quarterly_surveillance_cohort <- aaa_extract %>% 
  ...
  filter(date_screen < last_date) # 1,353 rows
...
quarterly_follow_up_appointments <- screened_cohort %>% 
  ...
  ungroup() %>% 
  rename(upi = upi_sc) %>% 
  filter(fy_quarter_qc %in% c("2020/21_4","2021/22_1","2021/22_2","2021/22_3"))# 1301 rows
...
quarterly_exclusions_appointments <- aaa_exclusions %>% 
  ...
  slice(n()) %>% 
  ungroup() %>% 
  filter(fy_quarter_qc %in% c("2020/21_4","2021/22_1","2021/22_2","2021/22_3"))
...
kpi_1.4b_assess <- template %>% left_join(kpi_1.4b_assess,  by = c("fy_due", "hbres" = "hbres_final"))
