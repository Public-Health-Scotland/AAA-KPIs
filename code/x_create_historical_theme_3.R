...
aaa_2.2_add_a <- aaa_2.2_add_a |> 
  ...
  mutate(fin_year = str_remove(fin_year, "FY_"),
         ...
         fin_year = str_remove(fin_year, "_recall_n"),
         fin_year = str_remove(fin_year, "_rate_p"),
         group = case_when(str_detect(group, "_selected") ~ "selected_n",
                           str_detect(group, "_no_result") ~ "no_result_n",
                           str_detect(group, "_rate_p") ~ "rate_p"),
         kpi = "KPI 2.2", .after = hb) |>
  glimpse()
...
aaa_kpi_historic <- rbind(aaa_2.1a, aaa_2.1b) |> 
