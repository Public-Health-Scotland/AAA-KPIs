...
# editied for re-run of GP Practices for 202309 with correct GP history
output_path <- "/PHI_conf/AAA/Topics/Screening/KPI/202409/output"

gp_prac_extract_date <- "23 November" # rough guess based on last year's outputs

current_gp_data_path <- paste0("/PHI_conf/AAA/Topics/Screening/KPI/202409", 
...
sr_path <- paste0("/PHI_conf/AAA/Topics/Screening/KPI/202409",
                  "/data/self_referrals_2223_correctGPhist.rds")
...
gp_prev <- read_rds(prev_gp_data_path) |> 
  rename(cohort_year_ww = cohort_year1,
         test_year_ww = tested2_year1, # edits for 2223 data
         percent_year_ww = percent_year1)  # edits for 2223 data
...
create_aaa_gp_outputs <- function(hb_name, financial_year, coverage_data, selfref_data, output_filepath, date_aaa_extracted, date_gp_extracted) {
  ...
  # 5: Write output ----
  output <- paste0(output_filepath, "/GP Practices 2223 correctGPhist/NHS ", hb_name, " Practice level ", simplify_fy(financial_year), ".xlsx")
